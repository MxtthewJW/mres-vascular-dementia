...
library(tidyverse) # includes pipe operator, ggplot, 
library(MASS) # for maths functions
library(broom) # tidy function for tables

#==================================================================
############ Method analysis
#==================================================================

# pandemic group age (mean and sd)
...
for (i in 1:total_samples) {
  sample <- i
  set.seed(1233 + i)
  dv <- c(rnorm(sample_size/2, 82.8125, 7.833422), rnorm(sample_size/2, 78.5625, 6.470124)) # here are each groups's stats properties
...
result %>% 
  filter(p.value < .05) %>%
  count() # 33/100 simulated datasets with p < .05
...
#==================================================================
############ Pandemic time period contrasts
#==================================================================
...
# Results tables with chi-square/fishers test
...
#==================================================================
############ Criteria comparison over separate time periods
#==================================================================
...
# Results tables with McNemar test 
...
#==================================================================
############ Criteria comparison over aggregated time periods
#==================================================================
...
#==================================================================
############ Element comparison over time periods
#==================================================================

# power analysis  

power.chisq.test(n = 25, w = 0.3, df = 1, sig.level = 0.05) # 32% power for brain imaging
power.chisq.test(n = 32, w = 0.3, df = 1, sig.level = 0.05) # 40% power for other elements

# results tables with chi-square/fisher's test ----
...
dsm4_cogdec <- matrix(c(13,3,12,4),
                     nrow = 2,
                     dimnames = list(c('Met','NoMet'),
                                     c('Pre-covid','Mid-covid'))) # dsm-4 from here
...
dsm5_cogdec <- matrix(c(15,1,13,3),
                     nrow = 2,
                     dimnames = list(c('Met','NoMet'),
                                     c('Pre-covid','Mid-covid'))) # dsm-5 from here
...
ninds_cogdec <- matrix(c(12,4,9,7),
                     nrow = 2,
                     dimnames = list(c('Met','NoMet'),
                                     c('Pre-covid','Mid-covid'))) # ninds from here
...
# results table with fishers test
...
#==================================================================
############ Criteria severity (level of diagnosis certainty)
#==================================================================

# power analysis (for both criteria separately as sample size differs)
